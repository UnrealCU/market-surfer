...
large_caps = ["^GSPC", "^STOXX", "AAXJ", "^TOPX"]
...
def get_OHLC_data(tickers, start_date = None, end_date = None):
    ...
    '''
    dataframes = []  # Collect all dataframes in a list
    ...
    final_OHLC_df = pd.concat(dataframes, ignore_index=False) if dataframes else pd.DataFrame()
...
if __name__ == "__main__":
    ...
    if os.path.isfile(OHLC_data_file):
        ...
        final_df = pd.concat([old_data_df, latest_data], ignore_index=False)
